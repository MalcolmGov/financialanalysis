...
async function main() {
  ...
  await db().insert(schema.prototypeVersions).values({
    ...
    promptText:
      "Style polish: wrapping sticky nav (no overflow-x), centered content column, prose measure/leading + AA contrast",
    ...
    JSON.stringify(
      {
        ...
        hasProseMeasure: assembledHtml.includes("--rs-prose"),
