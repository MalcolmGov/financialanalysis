/**
 * Phase 1 rebuild helper — soft gates so drafts persist; report Gate A/B status.
 *
 *   DATABASE_URL=... BLOB_READ_WRITE_TOKEN=... \
 *     pnpm exec tsx scripts/rebuild-phase1.ts <projectId> [...]
 */
import { RebuildSiteDraftError, rebuildProjectSiteDraft } from "../lib/rebuild-site-draft";

const ids = process.argv.slice(2);
if (!ids.length) {
  console.error("Usage: rebuild-phase1.ts <projectId> [...]");
  process.exit(1);
}
...
async function one(projectId: string) {
  ...
  try {
    const result = await rebuildProjectSiteDraft({
      projectId,
      note: "Phase 1 adaptive sitemap + never-drop commentary rebuild",
      hardFailGates: false,
    });
    console.log(
      JSON.stringify(
