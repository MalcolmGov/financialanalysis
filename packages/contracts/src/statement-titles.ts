...
function bookPrefix(opts: StatementTitleOpts): string {
  ...
  if (opts.entity === "group") {
    return isInterimDocKind(opts.docKind) ? "Condensed Group " : "Group ";
  }
  return isInterimDocKind(opts.docKind) ? "Condensed Consolidated " : "Consolidated ";
}
...
export function officialStatementEyebrow(
  docKind: FinancialDocKind,
  extras?: { dualEntity?: boolean; entity?: StatementEntity | null },
): string {
  ...
  if (docKind === "annual_audited") return "Consolidated financial statements";
